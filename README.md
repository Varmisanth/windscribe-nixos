...
<p align="center">
  <img alt="packaged" src="https://img.shields.io/badge/dynamic/regex?url=https%3A%2F%2Fraw.githubusercontent.com%2FVarmisanth%2Fwindscribe-nixos%2Fmain%2Fpkgs%2Fapp%2Fparams.nix&search=version%20%3D%20%22%28%5B0-9.%5D%2B%29%22&replace=%241&label=packaged&color=blueviolet">
...
Qt comes from a separate `nixos-24.11` input rather than from `nixpkgs`.

The `nixpkgs` input ships no client-side Wayland platform plugin in `qtwayland`.

Its plugin tree holds compositor libraries only, so the App would have no way to open a window.

The pin also matches the Qt that Windscribe builds against, which is 6.8.3.

One side effect is visible in the terminal.

The App links fontconfig 2.15 and warns about newer directives written by the system fontconfig.

Font selection is unaffected and the warnings go to stderr only.
