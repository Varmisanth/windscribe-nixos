...
| `app.startupBy` | enum | `"nix"` | Which mechanism starts the App with the session:<br>`"nix"` — systemd user unit<br>`"app"` — App's own autostart entry |
| `app.autoStart` | bool | `true` | Whether the App starts with the session at all |
...
| `settings` | submodule | `{ }` | Declarative preferences overlay — see [Settings](#settings) |
| `preferencesFile` | nullOr path | `null` | Path to a Windscribe Export JSON used as base state<br>`settings` overlay merges on top |
...
>
> `app.autoStart` decides whether the App starts with your session.
>
