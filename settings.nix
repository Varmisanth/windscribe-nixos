  ...
  mkAssertions =
    ...
    lib.optional (s.byApp != [ ]) (
      let
        ...
        listed = lib.concatStringsSep ", " unknown;
      in
      {
        assertion = unknown == [ ];
        message = "programs.windscribe.settings.byApp names options that do not exist: ${listed} — a misspelt entry filters nothing and the option keeps overwriting the App.";
      }
    )
    ...
    ++ lib.optionals (s.favoriteLocations != null) (
      map (
        ...
        let
          ...
          listed = lib.concatStringsSep ", " missing;
        in
        {
          assertion = missing == [ ];
          message = "programs.windscribe.settings.favoriteLocations entry is missing fields: ${listed} — App drops entries without type, id, and city.";
        }
      ) s.favoriteLocations
    )
      ...
      lib.optionals
        ...
        (
          map (
            ...
            {
              assertion = missing == [ ];
              message = ''programs.windscribe.settings.persistentState.networks entry is missing fields: ${listed} — networkOrSsid keys the per-network state, trustType decides whether the App auto-secures it instead of defaulting to "Secured".'';
            }
          ) s.persistentState.networks
        );
}
